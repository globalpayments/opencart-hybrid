<?php
class ModelExtensionPaymentGlobalPaymentsGatewayBase extends Model {
	private $gatewayName;
	...
	public function getMethod($address, $total) {
		$this->load->language('extension/payment/globalpayments_' . $this->gatewayName);

		$status            = true;
		$method_data       = array();
		$method_data_title = $this->config->get('payment_globalpayments_' . $this->gatewayName. '_title');
		if (empty($method_data_title)) {
			$method_data_title = $this->language->get('placeholder_title');
		}
		if ($status) {
			$method_data = array(
				'code'       => 'globalpayments_' . $this->gatewayName,
				'title'      => $method_data_title,
				'terms'      => '',
				'sort_order' => $this->config->get('payment_globalpayments_' . $this->gatewayName . '_sort_order')
			);
		}
	...
	public function getCards($customer_id, $gateway_id) {
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "globalpayments_card WHERE `customer_id` = '" . (int)$customer_id 
			. "' AND `gateway_id` = '" . $this->db->escape($gateway_id) . "' ORDER BY `token_id` DESC");
	...
	public function addTransaction($order_id, $gateway_id, $payment_action, $amount, $currency, $gatewayResponse) {
		$now = new DateTime();
		$timestamp = !empty($gatewayResponse->timestamp) ? $gatewayResponse->timestamp : $now->format('c');
