...
class ModelExtensionPaymentGlobalPaymentsGpiTrans extends Model {
	public function getMethod($address, $total) {
		...
		$status            = true;
		$method_data       = array();
		$method_data_title = $this->config->get('payment_globalpayments_gpitrans_title');
		if (empty($method_data_title)) {
			$method_data_title = $this->language->get('placeholder_title');
		}
		if ($status) {
			$method_data = array(
				'code'       => 'globalpayments_gpitrans',
				'title'      => $method_data_title,
				'terms'      => '',
				'sort_order' => $this->config->get('payment_globalpayments_gpitrans_sort_order')
			);
		}
