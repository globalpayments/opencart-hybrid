...
class ModelExtensionPaymentGlobalPaymentsAffirm extends Model {
	public function getMethod($address, $total) {
		...
		$status            = true;
		$method_data       = array();
		$method_data_title = $this->config->get('payment_globalpayments_affirm_title');
		if (empty($method_data_title)) {
			$method_data_title = $this->language->get('placeholder_title');
		}
		if ($status) {
			$method_data = array(
				'code'       => 'globalpayments_affirm',
				'title'      => $method_data_title,
				'terms'      => '',
				'sort_order' => $this->config->get('payment_globalpayments_affirm_sort_order')
			);
		}
